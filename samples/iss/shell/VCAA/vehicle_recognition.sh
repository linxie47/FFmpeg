...
DETECTION_MODEL=vehicle-detection-adas-0002-fp16
DETECTION_MODEL_PATH=$(GET_MODEL_PATH $DETECTION_MODEL )

CLASSIFY_MODEL=vehicle-attributes-recognition-barrier-0039-fp16
...
ffmpeg $HW_ACCEL -i ${FILE} -vf " \
  detect=model=$DETECTION_MODEL_PATH:model_proc=$DETECTION_MODEL_PROC_PATH:device=$DEVICE:nireq=24:configs=$CONFIGS, \
  classify=model=$CLASSIFY_MODEL_PATH:model_proc=$CLASSIFY_MODEL_PROC_PATH:device=$DEVICE:nireq=24" \
  -an -f null -